...
Важно для контрибьюторов: все .bat — только CRLF без BOM (иначе cmd.exe
ломается); ZAPRET.bat — cp866 без BOM (кириллица); списки .txt — без BOM.
Генераторы пишут именно так (UTF8NoBOM/ASCII); не используйте обычный
Set-Content -Encoding UTF8 для .bat/.txt.

Generate_Start_App_Via_Preset.bat — отдельный переносимый генератор
лаунчеров (релиз не трогает): окна проводника для .exe и .bat-пресета,
выход — <Exe>_ZapretExtra_<Preset>.bat в Generated_BatPreset_for_App\
рядом с собой. Сгенерированный файл на абсолютных путях (работает
отовсюду): проверки существования, ожидание winws до 20 сек с варнингом,
старт приложения, остановка по клавише.
