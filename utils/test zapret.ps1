...
function Get-IpsetStatus {
    $listFile = Join-Path $listsDir "ipsets\all.txt"
...
function Set-IpsetMode {
    param([string]$mode)
    $listFile = Join-Path $listsDir "ipsets\all.txt"
    $backupFile = Join-Path $listsDir "ipsets\all.test-backup.txt"
    if ($mode -eq "any") {
...
# Select top-level test type (standard vs DPI checkers vs combined)
function Read-TestType {
    while ($true) {
        ...
        Write-Host "  [3] Combined (Standard + DPI)" -ForegroundColor Gray
...
# Test profile: run all targets or one platform only
function Read-Profile {
    while ($true) {
        Write-Host ""
        Write-Host "Select test profile:" -ForegroundColor Cyan
        Write-Host "  [0] All targets" -ForegroundColor Gray
        Write-Host "  [1] Discord only (chats, media, voice filters)" -ForegroundColor Gray
        Write-Host "  [2] CloudFlare servers only" -ForegroundColor Gray
        Write-Host "  [3] Amazon servers only" -ForegroundColor Gray
...
function Test-TargetInProfile {
    param([string]$Name, [string]$Profile)
    if ($Profile -eq 'all') { return $true }
    if ($Profile -eq 'discord') { return ($Name -like 'Discord*') }
    if ($Profile -eq 'cloudflare') { return ($Name -like 'Cloudflare*' -or $Name -like 'CFPing*') }
    if ($Profile -eq 'amazon') { return ($Name -like 'Aws*') }
...
# Response/ping limits: answer too slow counts as failure (TIMEENDED / SLOW).
# Defaults: 2 sec response, 100 ms ping, 0 = no limit. Env overrides.
function Read-Limits {
    $resp = 2
    $ping = 100
    if ($env:TEST_RESP_LIMIT -match '^\d+$') { $resp = [int]$env:TEST_RESP_LIMIT }
    if ($env:TEST_PING_LIMIT -match '^\d+$') { $ping = [int]$env:TEST_PING_LIMIT }
    if (-not $env:TEST_RESP_LIMIT) {
        Write-Host ""
        $in = Read-Host "Max response time in seconds for HTTP probes (default: 2, 0 = no limit)"
        if ($in -match '^\d+$') { $resp = [int]$in }
    }
    if (-not $env:TEST_PING_LIMIT) {
...
while ($true) {
    $globalResults = @()
$testType = ""
if ($env:ZAPRET_TEST_TYPE -match '^(standard|dpi|combined)$') {
    $testType = $env:ZAPRET_TEST_TYPE
    Write-Host "[INFO] Test type preset by manager: $testType" -ForegroundColor Cyan
}
if (-not $testType) { $testType = Read-TestType }
$testProfile = 'all'
if ($testType -eq 'standard' -or $testType -eq 'combined') {
    $testProfile = Read-Profile
}
$testLimits = Read-Limits
$respLimitSec = $testLimits.RespSec
$pingLimitMs = $testLimits.PingMs
...
if ($testType -eq 'dpi' -or $testType -eq 'combined') {
...
if ($testType -eq 'standard' -or $testType -eq 'combined') {
    ...
    foreach ($key in $rawTargets.Keys) {
        if (Test-TargetInProfile -Name $key -Profile $testProfile) {
            $targetList += Convert-Target -Name $key -Value $rawTargets[$key]
        }
    }

    if ($targetList.Count -eq 0) {
        Write-Host "[ERROR] Profile '$testProfile' matched no targets. Check targets.txt." -ForegroundColor Red
        Wait-AnyKey -message "Press any key to exit..."
        exit 1
    }
    Write-Host "[INFO] Profile: $testProfile, targets in scope: $($targetList.Count)" -ForegroundColor Cyan
...
if ($testType -eq 'standard' -or $testType -eq 'combined') {
Write-Host "                 Profile: $($testProfile.ToUpper())" -ForegroundColor Cyan
}
...
try {
    ...
    $configNum++
    # unique key across authors: "Author\name.bat" for Presets, plain name in root
    $cfgKey = $file.Name
    if ($file.DirectoryName -ne $targetDir) {
        $cfgKey = (Split-Path $file.DirectoryName -Leaf) + '\' + $file.Name
    }
    Write-Host ""
    Write-Host "------------------------------------------------------------" -ForegroundColor DarkCyan
    Write-Host "  [$configNum/$($batFiles.Count)] $cfgKey" -ForegroundColor Yellow
    Write-Host "------------------------------------------------------------" -ForegroundColor DarkCyan
    ...
    if ($testType -eq 'standard' -or $testType -eq 'combined') {
        ...
        $scriptBlock = {
            param($t, $curlTimeoutSeconds, $respLimitSec, $pingLimitMs)
            ...
            if ($t.Url) {
                ...
                $baseArgs = @("-I", "-s", "-m", $curlTimeoutSeconds, "--connect-timeout", ([Math]::Min(2, $curlTimeoutSeconds)), "-o", "NUL", "-w", "%{http_code} %{time_total}", "--show-error")
                foreach ($test in $tests) {
                    try {
                        ...
                        if ($ok) {
                            $respTime = -1
                            if ($httpCode -match '^(\d{3})\s+([\d\.]+)$') {
                                $respTime = [double]$matches[2]
                            }
                            if ($respLimitSec -gt 0 -and $respTime -ge 0 -and $respTime -gt $respLimitSec) {
                                $httpPieces += "$($test.Label):TIMEENDED"
                            } else {
                                $show = ""
                                if ($respTime -ge 0) { $show = " t={0:N2}s" -f $respTime }
                                $httpPieces += "$($test.Label):OK   $show".TrimEnd() + "  "
                            }
                        } else {
            ...
            if ($t.PingTarget) {
                $ping = $null
                try {
                    # 4 echoes: avg RTT + packet loss (factors for unified preset)
                    ...
                    if ($rtts.Count -eq 0) {
                        ...
                        $avg = [math]::Round(($rtts | Measure-Object -Average).Average, 0)
                        $loss = [math]::Round($lost / 4 * 100, 0)
                        if ($pingLimitMs -gt 0 -and $avg -gt $pingLimitMs) {
                            $pingResult = "SLOW {0:N0} ms, loss {1}%" -f $avg, $loss
                        } else {
                            $pingResult = "{0:N0} ms, loss {1}%" -f $avg, $loss
                        }
                    }
                } catch {
                    $pingResult = "Timeout"
        ...
        foreach ($target in $targetList) {
            ...
            [void]$ps.AddArgument($respLimitSec)
            [void]$ps.AddArgument($pingLimitMs)
        ...
        foreach ($target in $targetList) {
            ...
            if ($res.IsUrl -and $res.HttpTokens) {
                foreach ($tok in $res.HttpTokens) {
                    ...
                    elseif ($tok -match "TIMEENDED") { $tokColor = "Red" }
                ...
                if ($res.PingResult -eq "Timeout" -or $res.PingResult -like "SLOW*") {
                    $pingColor = "Yellow"
                } else {
                    $pingColor = "Cyan"
                }
    ...
    if ($testType -eq 'dpi' -or $testType -eq 'combined') {
        ...
        $globalResults += @{ Config = $cfgKey; Type = 'dpi'; Results = $dpiResults }
    }
    ...
    # Analytics (dpi entries keyed separately so combined mode does not mix metrics)
    $analytics = @{}
    foreach ($res in $globalResults) {
        if ($res.Type -eq 'standard') {
            foreach ($targetRes in $res.Results) {
                $config = $res.Config
                if (-not $analytics.ContainsKey($config)) { $analytics[$config] = @{ OK = 0; ERROR = 0; UNSUP = 0; TIMEENDED = 0; PingOK = 0; PingFail = 0 } }
                if ($targetRes.IsUrl) {
                    foreach ($tok in $targetRes.HttpTokens) {
                        if ($tok -match "TIMEENDED") { $analytics[$config].TIMEENDED++ }
                        elseif ($tok -match "OK") { $analytics[$config].OK++ }
                ...
                if (($targetRes.PingResult -ne "Timeout") -and ($targetRes.PingResult -ne "n/a") -and ($targetRes.PingResult -notlike "SLOW*")) { $analytics[$config].PingOK++ } else { $analytics[$config].PingFail++ }
            }
        } elseif ($res.Type -eq 'dpi') {
            foreach ($targetRes in $res.Results) {
                $config = $res.Config + " [dpi]"
    ...
    foreach ($config in $analytics.Keys) {
        ...
        if ($a.ContainsKey('PingOK')) {
            $line = "{0} : HTTP OK: {1,3}, ERR: {2,3}, UNSUP: {3,3}, TIMEOUT: {4,3}, Ping OK: {5,3}, Fail: {6,3}" -f `
                $configPadded, $a.OK, $a.ERROR, $a.UNSUP, $a.TIMEENDED, $a.PingOK, $a.PingFail
        } else {
    ...
    # Determine best strategy (standard keys only; dpi keys end with " [dpi]")
    $bestConfig = $null
    $bestDpiConfig = $null
    $maxScore = 0
    $maxPing = -1
    ...
    foreach ($config in $analytics.Keys) {
        $a = $analytics[$config]
        if ($a.ContainsKey('PingOK')) {
            # timed-out responses count against the score
            $score = $a.OK - $a.TIMEENDED
            $pingScore = $a.PingOK
            ...
            if ($a.LIKELY_BLOCKED -lt $minBlocked -or ($a.LIKELY_BLOCKED -eq $minBlocked -and $a.OK -gt $maxDpiOk)) {
                $minBlocked = $a.LIKELY_BLOCKED
                $maxDpiOk = $a.OK
                $bestDpiConfig = $config
            }
        }
    }
    Write-Host ""
    ...
    if ($bestDpiConfig) {
        Write-Host "Best DPI config: $bestDpiConfig" -ForegroundColor Green
    }
    Write-Host ""
    ...
    [void]$resultLines.Add("Test type: $testType, profile: $testProfile, respLimit: ${respLimitSec}s, pingLimit: ${pingLimitMs}ms")
    foreach ($res in $globalResults) {
        $config = $res.Config
        $type = $res.Type
        $results = $res.Results
    ...
    try {
        $jsonFile = $resultFile -replace '\.txt$', '.json'
        $jsonObj = [PSCustomObject]@{
            date     = $dateStr
            testType = $testType
            profile  = $testProfile
            respLimit = $respLimitSec
            pingLimit = $pingLimitMs
            best     = $bestConfig
            bestDpi  = $bestDpiConfig
            analytics = $analytics
            results  = @($globalResults | ForEach-Object {
