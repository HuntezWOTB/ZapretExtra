...
:: LOAD USER LISTS =====================
:load_user_lists
set "DOM_PATH=%~dp0lists\domains\"
set "IPS_PATH=%~dp0lists\ipsets\"

if not exist "%IPS_PATH%exclude-user.txt" (
    echo 203.0.113.113/32>"%IPS_PATH%exclude-user.txt"
)
if not exist "%DOM_PATH%general-user.txt" (
    echo # Never leave this file empty>"%DOM_PATH%general-user.txt"
    echo domain.example.abc>>"%DOM_PATH%general-user.txt"
)
if not exist "%DOM_PATH%exclude-user.txt" (
    echo domain.example.abc>"%DOM_PATH%exclude-user.txt"
)
...
set "listFile=%~dp0lists\ipsets\all.txt"
...
echo Updating ipset (lists\ipsets\all.txt)...
