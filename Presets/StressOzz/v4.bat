@echo off
chcp 65001 > nul
:: StressOzz v4 (StressOzz/Zapret-Manager, adapted for winws)
...
start "zapret: v4 (StressOzz)" /min "%BIN%winws.exe" --wf-tcp=443,2053,2083,2087,2096,8443,%GameFilterTCP% --wf-udp=19294-19344,50000-50100,%GameFilterUDP% ^
...
--filter-tcp=443 --hostlist-exclude="%LISTS%domains\exclude.txt" --hostlist-exclude="%LISTS%domains\exclude-user.txt" --dpi-desync=multisplit --dpi-desync-split-seqovl=582 --dpi-desync-split-pos=1 --dpi-desync-split-seqovl-pattern="%BIN%stun.bin" --new ^
