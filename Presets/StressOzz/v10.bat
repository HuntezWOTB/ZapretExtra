@echo off
chcp 65001 > nul
:: StressOzz v10 (StressOzz/Zapret-Manager, adapted for winws)
...
start "zapret: v10 (StressOzz)" /min "%BIN%winws.exe" --wf-tcp=443,2053,2083,2087,2096,8443,%GameFilterTCP% --wf-udp=19294-19344,50000-50100,%GameFilterUDP% ^
...
--filter-tcp=443 --hostlist-exclude="%LISTS%domains\exclude.txt" --hostlist-exclude="%LISTS%domains\exclude-user.txt" --dpi-desync=fake,split2 --dpi-desync-split-pos=2 --dpi-desync-fake-tls="%BIN%tls_clienthello\tls_clienthello_www_google_com.bin" --dpi-desync-hostfakesplit-mod=host=maxcdn.bootstrapcdn.com --dpi-desync-fake-tls-mod=rnd,sni=maxcdn.bootstrapcdn.com --dpi-desync-fooling=ts --new ^
