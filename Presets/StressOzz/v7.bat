@echo off
chcp 65001 > nul
:: StressOzz v7 (StressOzz/Zapret-Manager, adapted for winws)
...
start "zapret: v7 (StressOzz)" /min "%BIN%winws.exe" --wf-tcp=443,2053,2083,2087,2096,8443,%GameFilterTCP% --wf-udp=19294-19344,50000-50100,%GameFilterUDP% ^
...
--filter-tcp=443 --hostlist-exclude="%LISTS%domains\exclude.txt" --hostlist-exclude="%LISTS%domains\exclude-user.txt" --dpi-desync=fake,multisplit --dpi-desync-split-seqovl=654 --dpi-desync-split-pos=1 --dpi-desync-fooling=badseq,badsum --dpi-desync-repeats=6 --dpi-desync-split-seqovl-pattern="%BIN%stun.bin" --dpi-desync-fake-tls="%BIN%stun.bin" --dpi-desync-badseq-increment=0 --new ^
