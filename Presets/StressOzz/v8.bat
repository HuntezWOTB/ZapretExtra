@echo off
chcp 65001 > nul
:: StressOzz v8 (StressOzz/Zapret-Manager, adapted for winws)
...
start "zapret: v8 (StressOzz)" /min "%BIN%winws.exe" --wf-tcp=443,2053,2083,2087,2096,8443,%GameFilterTCP% --wf-udp=19294-19344,50000-50100,%GameFilterUDP% ^
...
--filter-tcp=443 --hostlist-exclude="%LISTS%domains\exclude.txt" --hostlist-exclude="%LISTS%domains\exclude-user.txt" --dpi-desync=fake --dpi-desync-fooling=ts --dpi-desync-fake-tls="%BIN%tls_clienthello\tls_clienthello_4pda_to.bin" --dpi-desync-fake-tls-mod=none --new ^
