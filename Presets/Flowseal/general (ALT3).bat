...
--filter-tcp=80,443 --hostlist="%LISTS%domains\general.txt" --hostlist="%LISTS%domains\discord.txt" --hostlist="%LISTS%domains\google.txt" --hostlist="%LISTS%domains\cloudflare.txt" --hostlist="%LISTS%domains\amazon.txt" --hostlist="%LISTS%domains\instagram.txt" --hostlist="%LISTS%domains\general-user.txt" --hostlist-exclude="%LISTS%domains\exclude.txt" --hostlist-exclude="%LISTS%domains\exclude-user.txt" --ipset-exclude="%LISTS%ipsets\exclude.txt" --ipset-exclude="%LISTS%ipsets\exclude-user.txt" --dpi-desync=fake,hostfakesplit --dpi-desync-fake-tls-mod=rnd,dupsid,sni=ya.ru --dpi-desync-hostfakesplit-mod=host=ya.ru,altorder=1 --dpi-desync-fooling=ts --dpi-desync-fake-http="%BIN%tls_clienthello\tls_clienthello_max_ru.bin" --new ^
