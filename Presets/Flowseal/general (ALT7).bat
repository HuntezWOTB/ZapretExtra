...
--filter-tcp=80,443 --hostlist="%LISTS%domains\general.txt" --hostlist="%LISTS%domains\telegram.txt" --hostlist="%LISTS%domains\twitter.txt" --hostlist="%LISTS%domains\facebook.txt" --hostlist="%LISTS%domains\discord.txt" --hostlist="%LISTS%domains\google.txt" --hostlist="%LISTS%domains\cloudflare.txt" --hostlist="%LISTS%domains\amazon.txt" --hostlist="%LISTS%domains\instagram.txt" --hostlist="%LISTS%domains\general-user.txt" --hostlist-exclude="%LISTS%domains\exclude.txt" --hostlist-exclude="%LISTS%domains\exclude-user.txt" --ipset-exclude="%LISTS%ipsets\exclude.txt" --ipset-exclude="%LISTS%ipsets\exclude-user.txt" --dpi-desync=multisplit --dpi-desync-split-pos=2,sniext+1 --dpi-desync-split-seqovl=679 --dpi-desync-split-seqovl-pattern="%BIN%tls_clienthello\tls_clienthello_www_google_com.bin" --new ^
