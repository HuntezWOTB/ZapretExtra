...
--filter-tcp=80,443 --hostlist="%LISTS%domains\general.txt" --hostlist="%LISTS%domains\telegram.txt" --hostlist="%LISTS%domains\twitter.txt" --hostlist="%LISTS%domains\facebook.txt" --hostlist="%LISTS%domains\discord.txt" --hostlist="%LISTS%domains\google.txt" --hostlist="%LISTS%domains\cloudflare.txt" --hostlist="%LISTS%domains\amazon.txt" --hostlist="%LISTS%domains\instagram.txt" --hostlist="%LISTS%domains\general-user.txt" --hostlist-exclude="%LISTS%domains\exclude.txt" --hostlist-exclude="%LISTS%domains\exclude-user.txt" --ipset-exclude="%LISTS%ipsets\exclude.txt" --ipset-exclude="%LISTS%ipsets\exclude-user.txt" --dpi-desync=fake --dpi-desync-fake-tls-mod=none --dpi-desync-repeats=6 --dpi-desync-fooling=badseq --dpi-desync-badseq-increment=2 --dpi-desync-fake-http="%BIN%tls_clienthello\tls_clienthello_max_ru.bin" --new ^
